...
#include "Shared/UI/CultureSwitcherButtonWidget.h"

DEFINE_LOG_CATEGORY_STATIC(SharedModule, Log, All);
...
void UCultureSwitcherPanelWidget::GenerateCultureSwitcherButtons()
{
	...
	{
		UE_LOG(SharedModule, Error, TEXT("Failed to generate CultureSwitcher buttons | ButtonsPanel is not valid!"));
	}
	if (IsValid(this->CultureSwitcherWidgetType) == false)
	{
		UE_LOG(SharedModule, Error, TEXT("Failed to generate CultureSwitcher buttons | CultureSwitcherWidgetType is not valid!"));
	}
	...
	for (const auto& Culture : LocalizedCultures)
	{
		...
		CultureSwitcher->CultureChanged.BindUObject(
			this, &UCultureSwitcherPanelWidget::OnCultureChanged);
